...
char*
make_filename(const char* path, const char* dir, const char* filename, const char* extension)
{
    char* ret = NULL;
    int pos = 0;
...
// NOTE: caller must free the returned string!
char*
parse_format(const char* format, int tracknum, const char* year, const char* artist, const char* album, const char* title)
{
    unsigned i = 0;
    int len = 0;
    char* ret = NULL;
...
//  0 on succes
int
recursive_mkdir(char* path_and_name, char* base_dir, mode_t mode)
{
    int count;
    ...
    for (count = 0; count < path_and_name_length; count++)
    {
        if (pos[count] == '/' || pos[count] == '\\')
        {
            if (count == 0)
            {
                continue;          // skip first trailing slash
...
char*
get_unique_path(char* dir, char* file, const char* ext)
{
    char* path;
    char* file_new;
    unsigned int i = 0;
...
//  //  NOTE: caller must free the returned string!
char*
get_unique_filename(char* dev, char* dir, char* file, char* ext)
{
    struct stat stat_file;
