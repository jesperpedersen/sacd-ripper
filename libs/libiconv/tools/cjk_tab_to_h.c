 ...
 */
static bool
is_charset2uni_large (Encoding* enc)
{
    int row, col;
 ...
 */
static int
compact_large_charset2uni (Encoding* enc, unsigned int** urows, unsigned int* urowshift)
{
    unsigned int shift;
 ...
 */
static void
output_charset2uni_noholes_monotonic (const char* name, Encoding* enc)
{
 ...
 */
static void
output_uni2charset_sparse (const char* name, Encoding* enc, bool monotonic)
{
    bool pages[0x300];
    Block pageblocks[0x300];
    int npageblocks;
...
static int
row_byte_uhc_1 (int row)
{
    return 0x81 + row;
}
static int
col_byte_uhc_1 (int col)
{
    return (col >= 0x34 ? 0x4d : col >= 0x1a ? 0x47 : 0x41) + col;
}
static int
byte_row_uhc_1 (int byte)
{
...
do_uhc_1 (const char* name)
{
    ...
    invert(&enc);
    output_uni2charset_sparse(name, &enc, true);
}

/* UHC part 2: 0x{A1..C6}{41..5A,61..7A,81..A0} */

static int
row_byte_uhc_2 (int row)
{
    return 0xa1 + row;
}
static int
col_byte_uhc_2 (int col)
{
    return (col >= 0x34 ? 0x4d : col >= 0x1a ? 0x47 : 0x41) + col;
}
static int
byte_row_uhc_2 (int byte)
{
...
static int
row_byte_hkscs (int row)
{
    return 0x80 + row;
}
static int
byte_row_hkscs (int byte)
{
...
do_gb18030uni (const char* name)
{
    ...
    printf("static const unsigned char %s_bitmap[%d] = {\n ", name, (ranges_total + 7) / 8);
    {
        ...
        for (k = 0; k < ranges_count; k++)
        {
            ...
            if (i != (k + 1 < ranges_count ? ranges[k + 1].total : ranges_total))
            {
                abort();
            }
        }
