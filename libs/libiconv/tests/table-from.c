...
hexbuf (unsigned char buf[], unsigned int buflen)
{
    ...
    switch (buflen)
    {
        case 1: sprintf(msg, "0x%02X", buf[0]);
            break;
        case 2: sprintf(msg, "0x%02X%02X", buf[0], buf[1]);
            break;
        case 3: sprintf(msg, "0x%02X%02X%02X", buf[0], buf[1], buf[2]);
            break;
        case 4: sprintf(msg, "0x%02X%02X%02X%02X", buf[0], buf[1], buf[2], buf[3]);
            break;
        default: abort();
    }
    return msg;
...
static int
try (iconv_t cd, unsigned char buf[], unsigned int buflen, unsigned int* out)
{
