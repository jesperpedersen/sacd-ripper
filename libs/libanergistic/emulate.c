...
static int
emulate_instr(spe_ctx_t* ctx, uint32_t instr)
{
...
void
destroy_spe_context(spe_ctx_t* ctx)
{
    free(ctx->ls);
    free(ctx);
}
...
void
spe_in_mbox_write(spe_ctx_t* ctx, uint32_t msg)
{
    int done = 0;
...
void
spe_out_intr_mbox_status(spe_ctx_t* ctx)
{
    int done = 0;
...
uint32_t
spe_out_intr_mbox_read(spe_ctx_t* ctx)
{
    int done = 0;
