...
void
dump_ls(spe_ctx_t* ctx, const char* filename)
{
    FILE* fp;
    if (!ctx)
    {
...
void
fail(spe_ctx_t* ctx, const char* a, ...)
{
    char msg[1024];
    va_list va;
...
void
reg2ls(spe_ctx_t* ctx, uint32_t r, uint32_t addr)
{
...
void
ls2reg(spe_ctx_t* ctx, uint32_t r, uint32_t addr)
{
    addr &= LSLR & 0xfffffff0;
...
void
reg_to_byte(spe_ctx_t* ctx, uint8_t* d, int r)
{
    int i, j;
    for (i = 0; i < 4; ++i)
    {
...
void
byte_to_reg(spe_ctx_t* ctx, int r, const uint8_t* d)
{
    int i, j;
    for (i = 0; i < 4; ++i)
    {
...
void
reg_to_half(spe_ctx_t* ctx, uint16_t* d, int r)
{
    int i, j;
    for (i = 0; i < 4; ++i)
    {
...
void
half_to_reg(spe_ctx_t* ctx, int r, const uint16_t* d)
{
    int i, j;
    for (i = 0; i < 4; ++i)
    {
...
void
reg_to_bits(spe_ctx_t* ctx, uint1_t* d, int r)
{
    int i, j;
    for (i = 0; i < 4; ++i)
    {
...
void
bits_to_reg(spe_ctx_t* ctx, int r, const uint1_t* d)
{
    int i, j;
    for (i = 0; i < 4; ++i)
    {
...
void
reg_to_float(spe_ctx_t* ctx, float* d, int r)
{
    memcpy(d, ctx->reg[r], 16);
}

void
float_to_reg(spe_ctx_t* ctx, int r, const float* d)
{
    memcpy(ctx->reg[r], d, 16);
}

void
reg_to_double(spe_ctx_t* ctx, double* d, int r)
{
...
void
double_to_reg(spe_ctx_t* ctx, int r, const double* d)
{
    uint32_t* tmp = (uint32_t*)d;
    ctx->reg[r][0] = tmp[1];
    ctx->reg[r][1] = tmp[0];
