...
static void
safe_copy(char* dst, size_t cap, const char* src)
{
    if (!dst || cap == 0)
    {
        return;
...
static void
safe_copy_n(char* dst, size_t cap, const char* src, size_t nbytes)
{
    if (!dst || cap == 0)
    {
        return;
...
static void
safe_append(char* dst, size_t cap, const char* suffix)
{
    if (!dst || !suffix || cap == 0)
    {
        return;
...
//
int
utf8cpy(char* dst, char* src, int n)
{
...
//
char*
get_album_dir(scarletbook_handle_t* handle, int artist_flag)
{
    ...
    if (p_artist)
    {
        ...
        if (to_copy > MAX_DISC_ARTIST_LEN)
        {
            to_copy = MAX_DISC_ARTIST_LEN;
        }
    ...
    if (p_album_title)
    {
        ...
        if (to_copy > MAX_ALBUM_TITLE_LEN)
        {
            to_copy = MAX_ALBUM_TITLE_LEN;
        }
...
//  NOTE: caller must free the returned string!
char*
get_path_disc_album(scarletbook_handle_t* handle, int artist_flag)
{
...
char*
get_music_filename(scarletbook_handle_t* handle, int area, int track, const char* override_title, int performer_flag)
{
    char* c = NULL;
    ...
    if (c)
    {
        ...
        if (to_copy > MAX_TRACK_ARTIST_LEN)
        {
            to_copy = MAX_TRACK_ARTIST_LEN;
        }
...
char*
get_speaker_config_string(area_toc_t* area)
{
...
char*
get_frame_format_string(area_toc_t* area)
{
