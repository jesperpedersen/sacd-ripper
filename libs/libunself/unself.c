...
static void
read_section(uint32_t i, struct self_sec* sec)
{
    uint8_t* ptr;
...
static void
remove_shnames(uint64_t shdr_offset, uint16_t n_shdr, uint64_t shstrtab_offset, uint32_t strtab_size)
{
    uint16_t i;
    uint32_t size;
    struct elf_shdr s;
...
int
unself(const char* file_in, const char* file_out)
{
