...
void
decompress(uint8_t* in, uint64_t in_len, uint8_t* out, uint64_t out_len)
{
    z_stream s;
    int ret;
...
const char*
id2name(uint32_t id, struct id2name_tbl_t* t, const char* unk)
{
    while (t->name != NULL)
    {
        if (id == t->id)
...
static int
key_read(const char* path, uint32_t len, uint8_t* dst)
{
    FILE* fp = NULL;
    uint32_t read;
    int ret = -1;
...
//
int
elf_read_hdr(uint8_t* hdr, struct elf_hdr* h)
{
    int arch64;
...
void
elf_read_phdr(int arch64, uint8_t* phdr, struct elf_phdr* p)
{
    if (arch64)
    {
...
void
elf_read_shdr(int arch64, uint8_t* shdr, struct elf_shdr* s)
{
    if (arch64)
    {
...
void
elf_write_shdr(int arch64, uint8_t* shdr, struct elf_shdr* s)
{
    if (arch64)
    {
...
int
sce_decrypt_header(uint8_t* ptr, struct keylist_t* klist)
{
